...
export const handlerGroupModerationUnverify: BotPipeline<
	"message",
	DBSchema
> = async (message) => {
	const match = message.text?.match(/^\/unverify\s+([a-fA-F0-9]{32})$/i);
	...
	if (match) {
		...
		if (contest) {
			...
			sendMessage({
				...
				text: t("en", "message.group.moderation.unverify.text", {
					contest: slug,
				}),
				reply_parameters: {
